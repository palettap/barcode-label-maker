...
// Funzione condivisa per creare il PDF dell'etichetta\async function creaPdfEtichetta(imgData) {
  ...
  if (ultimaQuantita) doc.text(`Quantità: ${ultimaQuantita}`, T.quantita.x,    T.quantita.y,    null, null, T.quantita.align);
  if (ultimoPeso)      doc.text(`Peso: ${ultimoPeso}`,         T.peso.x,        T.peso.y,        null, null, T.peso.align);
